...
## Focused local business corpus — 16 September 2026

**Follow-up resolution:** The two cited company failures are now verified as repaired in local conversation `19fb0889-2d35-4280-8b53-74f3f4a727e6`. The original broad English sections 36/81/190 question returned complete coverage, recovered section 36 through one adjacent-recovery round, and achieved 11/11 supported claims (87.860 seconds). The Bangla certificate question, explicitly including more than 50 members, achieved 3/3 supported claims (26.993 seconds); separate coverage review was not needed/assessed for that turn. The response language matched each request.

The recurring certificate flag came from comparing numeric `50`/`৫০` with source word `পঞ্চাশ`. Comparison guards now recognize isolated number-word equivalents while leaving lexical/semantic verification and authority checks intact. Altered counts remain unverified, larger compound numbers are not truncated to their final component, and ambiguous standalone `বার` does not become twelve. Ten focused grounding tests and lint passed; only two new local messages were generated. No further configuration, source or production changes were made. Latency and unused evidence-list entries are separate remaining quality concerns, not reasons to describe these two cases as unresolved.

Local Test (`6cd8b5f4-7273-4b10-be84-064c130c8303`) now has three focused business sources: BIDA FAQ, Sale of Goods Act and the previously audited official Bangla Companies Act statutory sections. All are Ready and active; unrelated local documents and production were preserved. The local tax-only instructions were replaced with source-based business guidance, indexed-only evaluation and cross-language translation. A measured passage-context problem led to a local Custom profile with a 512-token passage window and 32,000-character context budget; the 8,192-token output reserve was already sufficient.

Local company checks recovered more legal conditions but still exposed missing adjacent section context, premature partial-answer acceptance and one wrong-language reply. Bounded recovery fixes and focused planning clarification were implemented; eight targeted backend cases passed. The local response trace confirms v20 is loaded. A focused section 36 follow-up now answers the deadline and private-company certificates, but the broad table case and some claim verification remain incomplete. This is not a company-readiness sign-off. Full source decisions, conversation IDs, measurements and remaining gaps are in [the local check record](artifacts/company-readiness/local-focused-check-2026-09-16.md).
...
## Company messaging deployment check — 16 September 2026, 4044fed

Two live messages in conversation `e7b12711-ca1f-48e7-9496-dddd228aad05` checked the deployed fixes. The English sections 36/81/190 table returned in 45.439 seconds with complete coverage and 9/10 supported claims. The prior AGM-row flag disappeared; the remaining unsupported claim was an added caveat asserting incomplete section 36(1) coverage. The Bangla rewrite returned three concise bullets with correctly remapped citations in 37.931 seconds, compared with the preceding deployment's refusal at 50.358 seconds. Its plan now contains only three factual requirements and recovery reports `recovered`. These two timings are observations, not a performance benchmark.

The Bangla response had 2/4 supported claims. Code inspection and focused reproductions identified comparison matching of `কম` inside `একমাত্র`, plus inconsistent treatment of spelled versus numeric duration quantities. Local changes require Bengali word boundaries and recognize source duration values in the comparison check. The existing duration guard still rejects changed quantities/units. Prompt v16 also limits caveats to material requested gaps and prevents inferring corpus absence from a selected excerpt. These refinements are not deployed and their live quality effect is unmeasured.

Ten focused backend tests passed (92 deselected), covering Bengali comparison cases, rejection of altered deadlines, duration normalization and independent table citation scope. No further live generations were run. Source generation 101 and policy revision 10 were observed; sources and tax settings were unchanged. Expanded diagnostic content is narrower than before but the inspector still exhibits some horizontal overflow on the narrow viewport; no claim of perfect responsive layout is made.

Provisional conversation-quality rating: **6/10**, an engineering judgment rather than a benchmark or percentage complete. Progress is concrete: readable tables/citations, a working higher output allowance, retained prior sources, successful Bangla follow-up, and corrected planning scope. Remaining barriers are uneven claim verification, 38–45 second latency in these cases, the previously recorded official company-source gaps, and lack of evidence for broad cross-domain reliability. No new tax regression test or broad legal-completeness certification was performed this turn. Evaluation dimensions follow relevance, groundedness and completeness as described in [Microsoft's RAG evaluation guidance](https://learn.microsoft.com/en-us/azure/foundry/concepts/evaluation-evaluators/rag-evaluators); the score itself is our assessment.

## Company messaging deployment check — 14a6a97

Two live messages were tested on 15 September 2026 in conversation `6a45608e-60b4-4576-8e7f-95b199cd6224`. The English sections 36/81/190 question returned a semantic table with working inline citation buttons, complete evidence coverage and 9/10 supported claims in 58.756 seconds. The prompt capacity panel confirms an 8,192-token output reserve. The remaining unsupported AGM sentence exposed table segmentation: citation inheritance was paragraph-wide, and a trailing pipe could hide the final citation. Local verification now isolates rows and ignores pipe-only fragments; each factual claim still requires verification.

The Bangla request to shorten that answer into three bullets retrieved and admitted all four prior citations, confirming deployed source recall. It nevertheless refused after 50.358 seconds: `repair_unavailable`, `invalid_model_response`, `coverage_unreviewed_missing_inputs`. The plan incorrectly included bullet count and retention of old citation numbers as source requirements. Planning/review instructions now distinguish presentation from factual dependencies and explicitly reserve missing-input classification for the separate input reviewer. This prompt correction has not yet been verified live and is not a claim that the follow-up is fixed in production.

Source generation 101 and policy revision 10 were observed; no source, lifecycle, relationship, model, or tax-policy changes were made. The live narrow-screen inspector expanded to 2,889 scroll pixels inside a 654-pixel panel when diagnostics were opened. Local CSS constrains its grid children. A separate renderer defect could loop on a bare list marker during streaming; the list parser now requires item content before entering its list loop.

Bounded backend verification passed 16 tests (183 deselected), including independent table rows, no borrowing from an uncited row's neighbour, existing prose citation inheritance, corrected planner references, and invalid coverage safeguards. One focused frontend test passed (12 skipped), including a trailing empty list marker; TypeScript, backend lint, formatting and diff checks passed. No further live queries were run. Full company readiness remains unconfirmed because the Bangla follow-up failed live and the official-source gaps recorded above remain unresolved. All changes in this section await deployment.
