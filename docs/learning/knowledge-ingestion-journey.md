...
> **Reading order:** this page → [Object storage](./object-storage-for-rag.md) → [Parsing](./document-parsing-and-extraction.md) → [Chunking](./text-chunking-for-rag.md) → [OCR](./ocr-fundamentals.md)
...
| 7c | `providers/implementations/pdf_extraction_workflow.py` | `.pdf` → page-level PyMuPDF → PDFium → optional OCR with quality scoring |
...
| 2 | `modules/knowledge/services/chunking_service.py` | Structure-aware pipeline: analyze → select → chunk → validate |
...
Config: `APE_CHUNKING__STRATEGY`, token-based limits (`TARGET_TOKENS`, `MAX_TOKENS`, …), `APE_OCR__ENABLED`, `APE_OCR__LANG`, `APE_KNOWLEDGE__MAX_UPLOAD_BYTES` in `core/config.py`.
